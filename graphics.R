...
Cambiar N por error estandard/Nest o algo aismilar, 
evalPerf(rec,10, .9)
...
u <- c(100, 80, 35, 30, 12)
ytest <- fitCatches(u, k = 5)

plot(cumsum(u), type = 'l', ylim = c(0,250))
points(ytest$fitted.values)
...
plot(p, type = 'l', ylim = c(0, 400))
points(cumsum(u))

recFitCatches <- function(){
    lapply.... fitCatches
}


plot(model)
points(capts, pch = 19, col = 'blue')

plot(pred$fit, type = 'b', lwd = 2)
...
##     n0 <- expit(parms[3])
##     N <- NInd + n0
##     pvec <- c(p[1]*p[2], (1-p[1])*p[2], p[1]*(1-p[2]), (1-p[1])*(1-p[2]))
##     -1*(lgamma(N+1)-lgamma(n0+1) + sum(c(y, n0)*log(pvec)))
## }

## res <- nlm(lik, c(0,0,1), hessian = TRUE)
